---
type: Spec
title: ABF-1 — the ADD Bundle Format, version 1.2-draft
description: >-
...
status: draft
version: 1.2-draft
supersedes: FORMAT.md@1.1-draft (2026-07-29 — amendments A19–A21 landed, ATG citations repaired)
generated: { by: claude/opus-5, at: 2026-07-29 }
relates_to: [ /PROPOSAL.md ]
---

# ABF-1 — ADD Bundle Format v1.2 (draft)

ABF-1 is a **profile of Open Knowledge Format v0.2**
(`github.com/GoogleCloudPlatform/knowledge-catalog/okf/SPEC.md`) plus **four declared
extensions**: a typed atomic task graph, a fragment grammar, an evidence-binding rule, and
a frozen node interface.
...
.add/                          # the OKF bundle root
  index.md                     # RESERVED — bundle config in frontmatter + a COMPILED body
                               ...
                               #   "1.1" (the skill's branch key — absent ⇒ a pre-ABF
...
A receipt is **stale** if any file matching the task's `scope:` has a modification time
later than the receipt's `at`. A stale receipt cannot earn a gate. This makes
"same-session receipt" mechanical instead of honour-system, and it kills the stale-green
failure — tests that passed before the last edit.

> ⚠ **Recorded assumption:** mtime is a sound freshness signal across checkouts and
> worktrees. If it proves unsound, freshness needs a content hash of the scope set, which
> costs a read of every scoped file. The predicate is isolated so it can be replaced
...
| `receipt_stale` | info | a receipt older than an in-scope edit (the *gate* refuses; the scan reports) |
